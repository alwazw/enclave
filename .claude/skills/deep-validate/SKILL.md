...
Host IP for this box is **$HOST_IP**. Run ALL of:
...
2. **Endpoint reachable** — the declared `endpoint` returns the expected
   ...
   `curl -fsS -o /dev/null -w '%{http_code}' http://$HOST_IP:<port>`
   and fetch real content: `curl -fsS http://$HOST_IP:<port>/<known-path> | head`
...
\$ curl -fsS -o /dev/null -w '%{http_code}' http://$HOST_IP:3030 -> 200
