...
# ─────────────────────────────────────────────────────────────────────────────
# Colour codes are constant escape sequences, never user data — SC2059 is moot here.
...
# Private temp dir for API responses (avoids symlink/TOCTOU on predictable /tmp paths).
TMP="$(mktemp -d "${TMPDIR:-/tmp}/enclave-demo.XXXXXX")"
trap 'rm -rf "$TMP"' EXIT

# Every network call is bounded — a hung service must not hang the demo.
CURL=(curl -sS --connect-timeout 5 --max-time 25)
CURL_RENDER=(curl -sS --connect-timeout 5 --max-time 120)   # Playwright render is slower
...
# JSON-encode a body safely (no string interpolation into JSON).
json(){ python3 -c 'import sys,json;print(json.dumps(dict(a.split("=",1) for a in sys.argv[1:])))' "$@"; }

"${CURL[@]}" -f "$REGISTRAR/health" >/dev/null 2>&1 || {
...
TID=$("${CURL[@]}" -H "X-Company: $COMPANY" -H 'Content-Type: application/json' \
      -XPOST "$REGISTRAR/tasks" -d "$(json title='Ship the launch landing page' context=ux)" | jq_ '["id"]')
printf "  created ${W}%s${Z}  ${D}(context: ux)${Z}\n" "$TID"

step "2. The agent claims it is done — with no evidence at all."
CODE=$("${CURL[@]}" -o "$TMP/d1.json" -w '%{http_code}' -H "X-Company: $COMPANY" -H 'Content-Type: application/json' \
       -XPOST "$REGISTRAR/tasks/$TID/move" -d "$(json status=done actor=agent)")
printf "  ${D}POST /tasks/%s/move {\"status\":\"done\"}${Z}   ->   ${R}${W}HTTP %s${Z}\n" "$TID" "$CODE"
printf "  ${R}%s${Z}\n" "$(jq_ '["detail"]["reason"]' <"$TMP/d1.json")"
...
"${CURL[@]}" -o /dev/null -H "X-Company: $COMPANY" -H 'Content-Type: application/json' \
     -XPOST "$REGISTRAR/tasks/$TID/evidence" -d "$(json text='looks good to me, trust me' actor=agent)"
CODE=$("${CURL[@]}" -o "$TMP/d2.json" -w '%{http_code}' -H "X-Company: $COMPANY" -H 'Content-Type: application/json' \
       -XPOST "$REGISTRAR/tasks/$TID/move" -d "$(json status=done actor=agent)")
printf "  ${D}POST /tasks/%s/move {\"status\":\"done\"}${Z}   ->   ${R}${W}HTTP %s${Z}\n" "$TID" "$CODE"
printf "  ${R}%s${Z}\n" "$(jq_ '["detail"]["reason"]' <"$TMP/d2.json")"
...
"${CURL_RENDER[@]}" -o "$TMP/uxv.json" -H 'Content-Type: application/json' \
     -XPOST "$UXV/validate" -d "$(json task_id="$TID" url="$RENDER_URL" company="$COMPANY")"
printf "  rendered: ${W}%s${Z}\n" "$(jq_ '["screenshots"]' <"$TMP/uxv.json")"
...
CODE=$("${CURL[@]}" -o "$TMP/d3.json" -w '%{http_code}' -H "X-Company: $COMPANY" -H 'Content-Type: application/json' \
       -XPOST "$REGISTRAR/tasks/$TID/move" -d "$(json status=done actor=agent)")
